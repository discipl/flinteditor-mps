...
import mu.KLogging
import org.discipl.flint.sources.test.clients.QuintorApiNsxTextLineClient.QuintorApiNsxTextLine
...
class QuintorTextLineTransformer : NewTextLineTransformer<QuintorApiNsxTextLine> {
    ...
    private fun transform(
        ...
        return when (textLine.type) {
            "simple" -> return TextLineImpl(
                textLine.id,
                textLine.tag,
                allLines.indexOf(textLine),
                textLine.text!!
            )
            "prefix-container" -> transformContainer(textLine, remainingLines, allLines)
            "container" -> transformContainer(textLine, remainingLines, allLines)
            else -> TODO("Can't deserialize text line: $textLine")
        }
    }
    ...
    private fun transformContainer(
        ...
        val titleLine = children.firstOrNull { it.type == "title" }
        val container = if (titleLine == null) {
            if (textLine.type.startsWith("prefix")) {
                PrefixContainer(
                    textLine.id,
                    textLine.tag,
                    allLines.indexOf(textLine),
